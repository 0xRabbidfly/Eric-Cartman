...
Batches top-20 orphaned Library notes (5 per batch), asks xAI to score
semantic relevance against candidate notes found via vault search, and
produces a diff-style proposal file for human review.
"""
import json
import os
import re
...
def discover_connections(
    ...
    """Find connection opportunities for top Library orphans via xAI.

    Returns list of {orphan_path, orphan_slug, suggested_links: [{slug, reason}]}.
    """
    api_key = get_xai_key()
    if not api_key:
        print("[connections] No xAI API key — skipping Phase 3.", file=sys.stderr)
        print("[connections] Set XAI_API_KEY env var or keyring automation/xai/api_key", file=sys.stderr)
        return []
    ...
    for i in range(0, len(library_orphans), BATCH_SIZE):
        ...
        # Rate-limit: pause between batches to avoid hitting API limits
...
def apply_connection_proposals(proposal_path: str, dry_run: bool = False, verbose: bool = False) -> int:
    """Apply a previously saved connection proposals file to the vault.

    Reads the proposals markdown file, parses suggested links, and appends
    them to the ## Related section of each orphan note.
    ...
    for section in sections[1:]:
        ...
        # Defensive parsing: validate matches before proceeding
...
# NOTE: _note_summary reads the entire note content then slices to `chars`.
# The Obsidian REST API does not support a content-length limit parameter,
# so we cannot avoid fetching the full note. This is acceptable for the
# typical vault note sizes (~1-10 KB), but could be a concern for very
# large notes (>100 KB). A future optimization could cache note sizes
# and skip oversized notes.
...
def _call_xai(api_key: str, model: str, prompt: str) -> dict:
    """Call xAI API with retry + exponential backoff (3 attempts, 2/4/8s waits).
