...
TOPIC_MOC_THRESHOLD = 12  # sections with more entries get a proposal
...
def reorganize_moc(dry_run: bool = False, verbose: bool = False) -> MOCResult:
    ...
    # Build set of all slugs in vault for dead-link detection
...
def _clean_moc(content: str, all_slugs: set, verbose: bool = False) -> tuple:
    """Remove dead wikilinks and intra-section duplicates. Returns (new_content, removed, deduped)."""
    ...
    for line in lines:
        ...
        slug = link_match.group(1).strip().split("|")[0]  # handle [[slug|display]]

        # Remove dead entries (target file no longer exists)
...
def _find_topic_moc_candidates(content: str) -> list:
    """Find sections with >TOPIC_MOC_THRESHOLD entries that merit a Topic MOC."""
    candidates = []
    lines = content.splitlines()
    current_section = None
    section_slugs = []
