...
You are analyzing all session logs in `.github/sessions/` to generate a comprehensive insights report, similar to Claude Code's `/insights` command but project-specific.

### Step 1: Discover Sessions

1. **Find all session files:**
   ```bash
   find .github/sessions -name "*.md" -type f
   ```
...
- Bash-heavy sessions (command-line work)
...
- **Command-line work:** Bash used 234 times
...
Based on your patterns, add these to your CLAUDE.md:
...
#### 1. <Claude Code feature recommendation>
...
*Generated by Claude Code `/insights-report` skill*
